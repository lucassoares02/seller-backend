...
const Client = {
  ...
  async postInsertPerson(req, res) {
    ...
    connection.query(queryInsert, (error, results) => {
      if (error) {
        result = false;
        console.log(`Post Insert Person (Client) Error: ${error} | ${cod} - ${nome}`);
        return;
      } else {
        response = results;
        return;
